...
from ..domain.physics import Coupling, Space, World
...
def build_world(floors: dict[str, dict]) -> tuple[World, dict[str, dict]]:
    ...
    for level, payload in floors.items():
        ...
        for room in payload.get("rooms", []):
            ...
            world.spaces[key_of(level, name)] = Space(
        ...
        # An edge joining two differently named nodes is an opening between two
        # rooms. That is how smoke gets from one to the other.
        ...
        for edge in graph.get("edges", []):
            ...
            if key_a not in world.spaces or key_b not in world.spaces:
                continue
        ...
        for (key_a, key_b), conductance in seen.items():
            world.couplings.append(Coupling(a=key_a, b=key_b, conductance=conductance))
...
def derive_exits(floors: dict[str, dict], exits: dict[str, list[str]]) -> dict[str, list[str]]:
    ...
    BuildSim's walkable graph stops at the floor slab, so an upper storey
    evacuates to its stairwells. Stairwells line up vertically, so the rooms
    nearest the known ground-floor exits are the right targets.
    """
